...
namespace AEX::NetProto {
    ...
    error_t IPv4Layer::parse(Dev::NetDevice_SP net_dev, const void* packet_ptr, size_t len) {
        ...
        if (header->destination != net_dev->ipv4_addr &&
            ...
            header->destination != BROADCAST_ADDR && (header->destination[0] & 0xF0) != 0b11100000)
            return error_t::EINVAL;
